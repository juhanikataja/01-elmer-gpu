...
!!$omp target data map(to: ngp, nd, Element, Element % TYPE, Basis, dBasisdx, DetJ, DiffCoeff, ReactCoeff, SourceCoeff, TimeCoeff) map(tofrom: STIFF, MASS, FORCE)
...
!!$omp end target data
    ...
    IF( Found ) THEN
    !!! THIS DOES NOT COMPILE WITH OMP TARGET IF THE LAST OPTIONAL ARGUMENT IS MISSING !!!
